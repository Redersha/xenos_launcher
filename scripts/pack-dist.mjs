 ...
 * Output structure:
 *   xenos-launcher/
 *   ├── xl          (launcher shell script / .cmd for Windows)
 *   ├── xl.js       (bundled app)
...
function getDirName(p) {
  ...
  return `xenos-launcher-${osName}-${archName}`;
}
...
copyFileSync(BUNDLE_JS, join(pkgDir, 'xl.js'));
...
"%~dp0node.exe" "%~dp0xl.js" %*
`;
  writeFileSync(join(pkgDir, 'xl.cmd'), cmdContent);
} else {
  // Unix shell launcher
  const shContent = `#!/bin/sh
DIR="$(cd "$(dirname "$0")" && pwd)"
exec "$DIR/node" "$DIR/xl.js" "$@"
`;
  const launcherPath = join(pkgDir, 'xl');
