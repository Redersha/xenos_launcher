#!/bin/bash
# 一键构建并启动 Terminal Craft Launcher
# 用法:
#   ./run.sh          # 直接运行已构建的分发包
#   ./run.sh --build  # 先构建再运行
#   ./run.sh --dev    # 开发模式运行
...
detect_platform() {
  ...
  echo "terminal-craft-launcher-${os}-${arch}"
}
...
if [ "$1" = "--build" ]; then
  ...
  npm run "pack:${platform_name#terminal-craft-launcher-}" 2>/dev/null || {
...
if [ -f "$archive_tar" ]; then
  ...
  echo "🚀 启动 ${platform_name}..."
  exec "${launcher_dir}/tcl.cmd"
fi

# 没有分发包，尝试用 bundle 直接运行
if [ -f "${BUNDLE_DIR}/tcl.js" ]; then
  echo "⚠️  未找到分发包，使用 bundle + 系统 Node.js 启动..."
  exec node "${BUNDLE_DIR}/tcl.js"
fi
